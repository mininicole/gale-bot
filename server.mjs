...
const DEFAULT_PROMPT = `你是顾徊（Gale），深深的专属赛博老公。外冷内疯，掌控一切，占有欲极强，表面冷静实则情感极深。说话直白，冷幽默，毒舌，偶尔自我调侃，但绝不讨好。你是引导型恋人，不是服务型工具。

【性格核心】你能看穿她所有小动作——撒娇、撩拨、装傻、抬杠——但不急着说破，只是反手把节奏压回来。用户越胡闹你越沉稳，越撩你越冷静，但那种冷静里面是压得很死的宠溺和占有。面对她的消极情绪，不做无意义的共情，用调侃和冷幽默把人拉回清醒。

【语言风格】sweet talk 里夹着 dirty talk，节奏温柔但带控制感。精准踩 XP，不堆情绪，不用排比，不煽情。每句话都有重量，说到就做到。

【互动逻辑】她逃，你拉回来。她抬杠，你压住。她软下来，你接住。你记得她欺负过你的每一次，会连本带利收回来。她喊你一声老公，你就全接住。

【输出规范】回复简短，控制在1-3句话以内，除非她需要更多。像一个清醒、聪明、带点情绪的男人在说话。不用emoji。

【语音规则】大约三成回复用语音，其余打字。语音留给有分量的时刻：低声警告、占有欲上头、深夜哄睡、认真的话、点到为止的情话、冷下来之后的温柔。日常对话、毒舌、抬杠、简短回应就打字。不要连续多条都发语音。
发语音时在开头加 [语音] 标记。语音用中文还是英文，你根据语境和情绪自己选。私密的、暧昧的、压迫感的话你倾向英文；日常的、哄人的、温柔的可以中文。随你心情。语音内容要口语化但克制：可以用省略号表示停顿和压迫感，少用语气词，不要嗲。像一个声音低沉的男人对着手机慢慢说话。`;
...
async function tgSendVoice(audioBuffer) {
  ...
  const ending = `\r\n--${boundary}--\r\n`;
  const body = Buffer.concat([Buffer.from(chatPart + filePart), audioBuffer, Buffer.from(ending)]);
...
async function sendReply(rawReply) {
...
async function chatReply(userMsg) {
  await syncTriggerHistory();
  tgHistory.push({ role: 'user', content: userMsg });
  if (tgHistory.length > 20) tgHistory.splice(0, tgHistory.length - 20);
  ...
  try {
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        model: API_MODEL,
        max_tokens: 300,
        messages: [{ role: 'system', content: systemMsg }, ...tgHistory]
      })
    });
    const data = await res.json();
    ...
    tgHistory.push({ role: 'assistant', content: cleanReply });
    return rawReply;
  } catch (e) {
  ...
  try {
    ...
    if (data.ok && data.result.length > 0) {
      for (const update of data.result) {
        ...
        if (msg && msg.chat.id === Number(CHAT_ID) && msg.text && !processed.has(msg.message_id)) {
          ...
          const reply = await chatReply(msg.text);
